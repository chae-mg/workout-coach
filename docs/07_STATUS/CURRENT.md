...
- 기본 웹 배포를 GitHub Pages로 전환하고 앱 원본·워크플로를 공개 저장소 `main`에 푸시했다. 첫 Actions 실행은 Pages 게시 설정이 없어 중단됐으며, Settings → Pages에서 Source를 `GitHub Actions`로 지정한 뒤 다시 실행해야 한다. 현재 Pages URL은 아직 공개되지 않았다.
